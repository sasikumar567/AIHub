...
function App() {
  const models = [
    { id: 'smollm2', name: 'SmolLM2', color: '#2ECC71' },
    { id: 'llama3.2', name: 'LLaMA 3.2', color: '#3498DB' },
    { id: 'deepseek-r1', name: 'DeepSeek R1 (1.5B)', color: '#E67E22' }
  ];

  const [sharedPrompt, setSharedPrompt] = useState('');
  const [responses, setResponses] = useState(() => {
    const initial = {};
    models.forEach(model => initial[model.id] = '');
    return initial;
  });
  const [loading, setLoading] = useState(() => {
    const initial = {};
    models.forEach(model => initial[model.id] = false);
    return initial;
  });
  ...
  const fetchModelResponse = useCallback(async (model, prompt) => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/ai/${model}/${encodedPrompt}`);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setLoading(() => {
      const updated = {};
      models.forEach(model => updated[model.id] = true);
      return updated;
    });
    ...
    setResponses(() => {
      const updated = {};
      models.forEach(model => updated[model.id] = 'Loading...');
      return updated;
    });
